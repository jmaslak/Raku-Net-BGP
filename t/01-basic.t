...
subtest 'Basic Class Construction', {
    my $bgp = Net::BGP.new(:my-asn(65000));
    ok $bgp, "Created BGP Class";
    is $bgp.port, 179, 'Port has proper default';

    $bgp = Net::BGP.new( port => 1179, my-asn => 65000 );
    is $bgp.port, 1179, 'Port is properly set to 1179';

    $bgp = Net::BGP.new( port => Nil, my-asn => 65000 );
    is $bgp.port, 179, 'Port is properly set to 179 by Nil';

    dies-ok { $bgp.port = 17991; }, 'Cannot change port';

    dies-ok { $bgp = Net::BGP.new( port =>    -1, my-asn => 65000 ); }, '< 0 port rejected';
    dies-ok { $bgp = Net::BGP.new( port => 65536, my-asn => 65000 ); }, '>65535 port rejected';

    dies-ok { $bgp = Net::BGP.new( foo => 1, my-asn => 65000 ); }, 'Non-existent attribute causes failure';
